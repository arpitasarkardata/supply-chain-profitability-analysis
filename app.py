...
@st.cache_data
def load_data():
    df = pd.read_csv("APL_Logistics.csv", encoding="latin1", low_memory=False)
...
try:
    ...
    st.error("⚠️ APL_Logistics.csv not found. Make sure it is in the same folder as app.py.")
    st.stop()

# ── Sidebar Filters ───────────────────────────────────────────────────────────
st.sidebar.title("🔍 Filters")
st.sidebar.markdown("Use filters to drill down into the data.")
...
with tab4:
    ...
    with col1:
        ...
        fig = px.scatter(sample, x="Order Item Discount Rate", y="Order Item Profit Ratio",
                         color="Customer Segment", opacity=0.5, trendline="ols",
